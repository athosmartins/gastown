...
if [ -z "$ALL_CANDIDATES_TIER" ]; then
  ...
  while IFS= read -r rig_path; do
    ...
    # Inject _bead_db so dispatch_one writes labels to the correct DB (ga-przl fix)
...
dispatch_one() {
  ...
  local STORY_ID STORY_TITLE STORY_PRIORITY STORY_LABELS STORY_RIG BEAD_DB
  ...
  # All bead label ops use BEAD_DB so in-flight/dispatching land in the right DB.
...
TASK
  ...
  if [ "$DRY_RUN" != "1" ]; then
    ...
    bd -C "$BEAD_DB" label remove "$STORY_ID" "pilot:dispatching" -q 2>/dev/null || true
    bd -C "$BEAD_DB" label add    "$STORY_ID" "story:in-flight"   -q 2>/dev/null || true
...
No human review required."
    ...
    bd -C "$BEAD_DB" comment "$STORY_ID" "$DISPATCH_COMMENT" \
