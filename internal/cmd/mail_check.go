...
func runMailCheck(cmd *cobra.Command, args []string) error {
	...
	if mailCheckInject {
		...
		if unread > 0 {
			...
			// Defense-in-depth: filter out messages already delivered in a previous session.
			// Plugin dispatch mails (and other task mails) accumulate as open beads if not
			// explicitly closed, causing re-injection on every UserPromptSubmit turn.
			// Upstream's gt-dog-done plugin-mail-archive (#3541) handles the normal flow;
			// this filter additionally protects against agents that crash before `gt done`.
