...
// stubDaemonRunning makes the DoltServerReachable check act as if the daemon
// is up so it actually runs (instead of taking the early-return skip path).
// Restores the original probe on test cleanup.
func stubDaemonRunning(t *testing.T) {
	t.Helper()
	orig := daemonIsRunning
	daemonIsRunning = func(string) (bool, int, error) { return true, 0, nil }
	t.Cleanup(func() { daemonIsRunning = orig })
}
