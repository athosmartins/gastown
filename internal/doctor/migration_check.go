...
// daemonIsRunning is the daemon liveness probe used by DoltServerReachableCheck.
// Indirected as a package var so tests can stub it; defaults to daemon.IsRunning.
var daemonIsRunning = func(townRoot string) (bool, int, error) {
	return daemon.IsRunning(townRoot)
}
...
func (c *DoltServerReachableCheck) Run(ctx *CheckContext) *CheckResult {
	...
	if running, _, err := daemonIsRunning(ctx.TownRoot); err != nil || !running {
		return &CheckResult{
			Name:     c.Name(),
			Status:   StatusOK,
