...
// FetchCompletedBeads returns beads closed in the last 24 hours.
func (f *LiveConvoyFetcher) FetchCompletedBeads() ([]CompletedBead, error) {
	out, err := f.runBdCmd(f.townBeads, "list", "--status=closed", "--json", "--limit=50")
	if err != nil {
		return nil, err
	}
	...
	var result []CompletedBead
	for _, item := range items {
		updatedStr, _ := item["updated_at"].(string)
		if updatedStr == "" {
			continue
		}
		t, err := time.Parse(time.RFC3339, updatedStr)
		if err != nil {
			t, err = time.Parse("2006-01-02T15:04:05Z", updatedStr)
			if err != nil {
				continue
			}
		}
		...
		priFloat, _ := item["priority"].(float64)
		assignee, _ := item["assignee"].(string)
		result = append(result, CompletedBead{
			ID:       id,
			Title:    title,
			ClosedAt: formatMailAge(time.Since(t)),
			Priority: int(priFloat),
			Assignee: formatAgentAddress(assignee),
		})
	}
	return result, nil
}

// FetchReadyQueue returns open beads with no assignee, sorted by priority.
func (f *LiveConvoyFetcher) FetchReadyQueue() ([]ReadyQueueRow, error) {
	out, err := f.runBdCmd(f.townBeads, "list", "--status=open", "--json", "--limit=30")
	if err != nil {
		return nil, err
	}
	...
	priEmoji := map[int]string{0: "🔴", 1: "🟠", 2: "🔵", 3: "⚪", 4: "⚫"}
	priClass := map[int]string{0: "pq-p0", 1: "pq-p1", 2: "pq-p2", 3: "pq-p3", 4: "pq-p4"}

	var result []ReadyQueueRow
	for _, item := range items {
		assignee, _ := item["assignee"].(string)
		if assignee != "" {
			continue
		...
		if itemType == "convoy" || itemType == "epic" || itemType == "milestone" {
			continue
		}
		id, _ := item["id"].(string)
		title, _ := item["title"].(string)
		...
		emoji, ok := priEmoji[pri]
		if !ok {
			emoji = "⚫"
		}
		class, ok := priClass[pri]
		if !ok {
			class = "pq-p4"
		}
		result = append(result, ReadyQueueRow{
			ID:       id,
			Title:    title,
			Priority: pri,
			PriClass: class,
			PriEmoji: emoji,
			PriLabel: fmt.Sprintf("P%d", pri),
			Age:      age,
		})
		if len(result) >= 20 {
			break
...
// FetchActiveWork returns in-progress beads with staleness info.
func (f *LiveConvoyFetcher) FetchActiveWork() ([]ActiveWorkRow, error) {
	out, err := f.runBdCmd(f.townBeads, "list", "--status=in_progress", "--json", "--limit=20")
	if err != nil {
		return nil, err
	}
	...
	for _, item := range items {
		...
		if updatedStr != "" {
			if t, err := time.Parse(time.RFC3339, updatedStr); err == nil {
				minutesIdle = int(time.Since(t).Minutes())
			}
		}

		statusLabel := "Active"
		statusClass := "aw-active"
		statusEmoji := "🟢"
		if minutesIdle >= 30 {
			statusLabel = "Stalled"
			statusClass = "aw-stalled"
			statusEmoji = "🔴"
		...
		agentShort := formatAgentAddress(assignee)

		result = append(result, ActiveWorkRow{
			ID:          id,
			Title:       title,
			Assignee:    assignee,
...
// FetchMetrics computes productivity metrics from recently closed beads.
func (f *LiveConvoyFetcher) FetchMetrics() (*MetricsData, error) {
	out, err := f.runBdCmd(f.townBeads, "list", "--status=closed", "--json", "--limit=200")
	if err != nil {
		return nil, err
	}
	...
	priWeight := map[int]int{0: 3, 1: 2, 2: 1}

	metrics := &MetricsData{}
	var dailyMap [7]struct{ pts, count int }
	var totalLeadHrs float64
	var leadCount int
	...
	for _, item := range items {
		...
		closedAt, err := time.Parse(time.RFC3339, updatedStr)
		if err != nil {
			closedAt, err = time.Parse("2006-01-02T15:04:05Z", updatedStr)
			if err != nil {
				continue
			}
		}
		if closedAt.Before(sevenDaysAgo) {
			continue
		}

		priFloat, _ := item["priority"].(float64)
		pri := int(priFloat)
		weight := priWeight[pri]
		if weight == 0 {
			weight = 1
		}

		// Today's points
		if !closedAt.Before(todayStart) {
			metrics.TodayPoints += weight
			switch pri {
			case 0:
				metrics.P0Count++
				metrics.P0Points += weight
			case 1:
				metrics.P1Count++
				metrics.P1Points += weight
			case 2:
				metrics.P2Count++
				metrics.P2Points += weight
			}
		}

		// Daily history (day index 0=today, 6=6 days ago)
		dayIdx := int(todayStart.Sub(time.Date(closedAt.Year(), closedAt.Month(), closedAt.Day(), 0, 0, 0, 0, closedAt.Location())).Hours() / 24)
		if dayIdx >= 0 && dayIdx < 7 {
			dailyMap[dayIdx].pts += weight
			dailyMap[dayIdx].count++
		}
		...
		if createdStr != "" {
			if createdAt, err := time.Parse(time.RFC3339, createdStr); err == nil {
				totalLeadHrs += closedAt.Sub(createdAt).Hours()
				leadCount++
			}
		}
	}

	// Build 7-day history (oldest first for the chart)
	dayNames := []string{"Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"}
	for i := 6; i >= 0; i-- {
		d := todayStart.AddDate(0, 0, -i)
		metrics.DailyHistory = append(metrics.DailyHistory, DayHistory{
			Day:    dayNames[d.Weekday()],
			Points: dailyMap[i].pts,
			Count:  dailyMap[i].count,
		})
	}

	// 7-day average
	var totalSevenDay int
	for _, d := range dailyMap {
		totalSevenDay += d.pts
	}
	metrics.SevenDayAvgPts = float64(totalSevenDay) / 7.0

	// Lead time
	if leadCount > 0 {
		metrics.AvgLeadTimeHrs = totalLeadHrs / float64(leadCount)
	}
	...
	if total7DayCount > 0 {
		metrics.HighPrioPct = float64(highPri7DayCount) / float64(total7DayCount) * 100
	}

	// Trend arrow (compare today vs 7-day avg)
	if metrics.TodayPoints > int(metrics.SevenDayAvgPts*1.1) {
		metrics.TrendArrow = "↑"
	} else if metrics.TodayPoints < int(metrics.SevenDayAvgPts*0.9) {
		metrics.TrendArrow = "↓"
	} else {
		metrics.TrendArrow = "→"
	}
	...
	// Lead time trend
	if metrics.AvgLeadTimeHrs < 24 {
		metrics.LeadTimeTrend = "Improving"
	} else if metrics.AvgLeadTimeHrs < 72 {
		metrics.LeadTimeTrend = "Stable"
	} else {
		metrics.LeadTimeTrend = "Degrading"
	}

	// High priority status
	if metrics.HighPrioPct >= 40 {
		metrics.HighPrioStatus = "On track"
	} else {
		metrics.HighPrioStatus = "Below target"
	}
