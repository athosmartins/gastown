...
#
# Section 1 (bug repro) needs the real `gt` binary and SKIPs cleanly if it's
# absent; sections 2-3 use plain `git -c` and must always run regardless of
# `gt`'s presence — a skip must never look like a pass in the summary line.
#
...
COMMANDS_FILE="$(cd "$HERE/../../commands" && pwd)/crew-commit.md"
...
# containing the IDENTITY= assignment (crew-commit SKILL.md's Step 5, or the
# equivalent block in commands/crew-commit.md's Step 4).
...
CMD_SRC="$(extract_identity_block "$COMMANDS_FILE")"
if [ -z "$CMD_SRC" ]; then
  echo "FATAL: could not extract the IDENTITY= commit recipe from $COMMANDS_FILE"
  exit 1
fi
...
#    Illustrative only: needs the real `gt` binary, unlike sections 2-3
#    below (plain `git -c ...`, no `gt` dependency), which verify the
#    actual shipped fix and always run below regardless of this one.
# ═════════════════════════════════════════════════════════════════════════
if command -v gt >/dev/null 2>&1; then
  ...
  AFTER_SHA=$(git -C "$WORK" rev-parse HEAD)
  if [ "$AFTER_SHA" = "$BEFORE_SHA" ]; then
    bad "bare 'gt commit' did not create a new commit at all (HEAD stayed at $BEFORE_SHA) — cannot confirm the bug reproduces this way; check gt's exit status/flags in this environment"
  else
  ...
  skip "bare 'gt commit' repro — gt binary not on PATH in this environment (sections 2+3 below don't need gt and still run)"
fi
...
echo "-- commands/crew-commit.md Step 4 recipe produces the agent's own identity --"
stage_change
( cd "$WORK" && eval "$CMD_SRC" ) >/dev/null 2>&1
CMD_AUTHOR=$(git -C "$WORK" log -1 --format='%an <%ae>')
if [ "$CMD_AUTHOR" = "test-agent-alias <test-agent-alias@gascity.local>" ]; then
  ok "commands/crew-commit.md recipe correctly authors as 'test-agent-alias <test-agent-alias@gascity.local>'"
else
  bad "commands/crew-commit.md recipe did not produce the agent identity — got '$CMD_AUTHOR'"
fi
...
echo ""
echo "Results: $P passed, $F failed, $S skipped"
