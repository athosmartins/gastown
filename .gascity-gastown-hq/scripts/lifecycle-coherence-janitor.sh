...
case "\$a" in
  ...
  *"show t-scope-review"*)                       echo '[{"id":"t-scope-review","status":"open","labels":["delivery:partial","gate:passed","scope:needs-review"],"metadata":{"gc.routed_to":"pool"}}]' ;;
...
GITSHIM
  ...
  # ga-6dpoa: the scope guard's OWN label (scope:needs-review) must never collide with
  # R7's non-implementable matcher — it signals "a human should confirm full scope
  # delivery", not "this bead cannot be automated". Unlike gate:needs-human:partial-delivery
