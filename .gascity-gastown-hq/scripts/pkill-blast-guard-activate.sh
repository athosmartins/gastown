#!/usr/bin/env bash
# pkill-blast-guard-activate.sh (ga-tje7u, supersedes ga-jo3xl) —
# idempotently registers pkill-blast-guard.py as a PreToolUse:Bash hook in
# a Claude Code settings.json, composing with whatever hooks are already
# there.
#
# WHY: .gc/ is gitignored city-wide -- so the live settings.json this guard
# must land in can never itself be a git commit. This script is the
# git-tracked, reviewable, re-runnable substitute: run it once to
# activate, and again any time `gc init`/`gc start` resets .gc/settings.json
# back to the embedded default. It replaces only its OWN prior PreToolUse
# entry (matched by this script's basename in the registered command),
# never any other hook.
#
