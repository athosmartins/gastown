...
# ga-h6trx3 item 2: self-heal this Pilot's OWN expired sling holds at the
# START of every sweep, on Pilot's own (faster) cadence — independent of
# whether the rest of this sweep proceeds or pauses below (quota/RAM/quiet-
# hours). See _pilot_selfheal_expired_slings' own header comment.
_pilot_selfheal_expired_slings "$GC_CITY"
...
_pilot_hold_or_escalate() {
  ...
  _phe_cur=$(printf '%s' "$_phe_labels" | jq -r --arg p "$_phe_prefix" \
    '(. // []) | map(select(startswith($p)) | ltrimstr($p) | select(test("^[0-9]+\\z")) | tonumber) | if length > 0 then max else 0 end' \
    2>/dev/null)
...
# unchanged before doing anything else, so it can never alter what actually
# dispatches regardless of where it sits in a filter pipe (verified by this
# bead's selftest's AC3-style differential). All diagnostics/errors go to fd 2
# explicitly (`log ... >&2`) — this function's fd 1 is someone else's pipe,
# exactly like _log_exclusions already has to be careful about.
#
# ⚠️ MUST be called BEFORE _filter_candidates, NOT chained after it (ga-fgdmol,
# fixed 2026-08-16 — mirrors the identical fix already applied to
# _reconcile_empty_description_signal directly below for ga-iu3xc5). At all 19
# real call sites this was originally wired `_filter_candidates |
# _reconcile_text_veto_labels "<db>"` — _filter_candidates' own select already
# drops a text-veto-matching bead from its stdout before this function ever
# saw it as stdin, making the "add" branch below structurally unreachable in
# production even though every isolated-invocation selftest scenario (this
# function fed raw, unfiltered fixtures directly) passed green throughout.
# Reordered to `_reconcile_text_veto_labels "<db>" | _filter_candidates` at
# all 19 sites; see pilot-dispatcher.text-veto-label.selftest.sh Scenario
# 9-11 for the chained-reachability proof (positive, negative control, and a
# structural check against the live call sites themselves).
...
# _reconcile_empty_description_signal <db>   (reads candidate JSON array from
# stdin; transparent pass-through — ALWAYS echoes stdin unchanged, side
# effects only, same contract as _reconcile_text_veto_labels directly above)
#
# ga-iu3xc5: _filter_candidates' empty-description veto (~L2265 below, mirrored
# for logging only in its trace block via test("\\S")|not) is label-blind
# exactly like the 4 text vetoes _reconcile_text_veto_labels exists to fix —
# a bead excluded for it gets ONE "[pilot] EXCLUÍDO ... empty-description" log
# line nobody reads and otherwise looks like a healthy bead patiently awaiting
# its turn. Measured 15/08: 6 beads (5 born on the dc rig, 1 on wa) sat in
# exactly this limbo, re-excluded silently every sweep, one for over a week.
#
# ⚠️ MUST be called BEFORE _filter_candidates, NOT chained after it the way
# _reconcile_text_veto_labels is (see every call site below: this function's
# call is inserted immediately to the LEFT of "_filter_candidates", not the
# right). Verified empirically, not just by reading: piping a bead matching
# a text-veto pattern through the REAL production chain — _filter_candidates
# feeding straight into _reconcile_text_veto_labels, db argument aside —
# produces ZERO bd calls. _filter_candidates' own select already drops the
# matching bead from its stdout, so nothing chained downstream of it ever
# sees the bead to label.
# That was a pre-existing, separate latent defect in _reconcile_text_veto_labels'
# OWN wiring for its 4 patterns (filed separately as ga-fgdmol, out of scope
# for this bug at the time — different function, different risk profile; since
# fixed 2026-08-16 with the identical reorder applied here). This function
# avoids repeating that mistake by sitting upstream of the select instead,
# where it always sees the full pre-filter candidate set.
#
# blocked:sem-descricao (not a pilot:text-veto:* label) deliberately reuses
# the painel's ALREADY-existing generic `blocked:` prefix handling
# (_travada_reason in painel_visibilidade.py, precedent blocked:wrong-store-
# needs-rehome-hq) — a tagged bead surfaces in 🔒 Travadas for free, no painel
# change is strictly required to satisfy AC3 (a companion change there adds a
# more specific reason string, mirroring that same precedent).
#
# One-time notice, not every-sweep noise: the comment + Mayor mail fire ONLY
# on the add transition (label absent → present), gated by the SAME label
# this function stamps — no separate counter needed. A bead that gets a
# description, self-clears, then goes empty again later gets a fresh notice
# on the next add transition, which is correct: a NEW instance of the
# problem, not a repeat of the old one.
_reconcile_empty_description_signal() {
  local _red_db="$1"
  local _red_in; _red_in=$(cat)
  printf '%s' "$_red_in"
  [ -z "$_red_db" ] && return 0
  ...
  _red_actions=$(printf '%s' "$_red_in" | jq -r --arg lbl "$_red_lbl" '
      ...
      | @tsv
    ' 2>/dev/null)
  [ -z "$_red_actions" ] && return 0

  printf '%s\n' "$_red_actions" | while IFS=$'\t' read -r _red_id _red_verb _red_creator; do
    [ -z "$_red_id" ] && continue
    if [ "$DRY_RUN" = "1" ]; then
      log "[pilot-empty-desc] WOULD $_red_verb $_red_lbl on $_red_id" >&2
      continue
    fi
    # ga-iu3xc5: gate the one-time comment/mail on the label write actually
    # succeeding, not just attempted — swallowing this exit code the same
    # way the label call itself swallows transient bd errors (`|| true`)
    # would let a bd hiccup send the notification while $has stays false,
    # so the NEXT sweep sees an unlabeled-but-already-notified bead and
    # fires a second comment+mail for the same occurrence (defeats AC2/AC4's
    # one-time guarantee). On failure, skip notifying and let the next
    # sweep retry the add from scratch — same self-heal-via-next-sweep
    # shape this file already uses elsewhere (e.g. gate-done's marker
    # readback self-heal).
    if bd -C "$_red_db" label "$_red_verb" "$_red_id" "$_red_lbl" -q >/dev/null 2>&1; then
      log "[pilot-empty-desc] $_red_verb $_red_lbl on $_red_id" >&2
    else
      log "[pilot-empty-desc] FAILED to $_red_verb $_red_lbl on $_red_id (bd label error) — skipping notification, will retry next sweep" >&2
      continue
    fi
    if [ "$_red_verb" = "add" ]; then
  ...
  _cf_out=$(printf '%s' "$_cf_in" | jq --arg self "$SELF_BEAD_ID" --argjson preapproval "$_FILTER_PREAPPROVAL_LABELS" \
    ...
    '[.[] | select(
        ...
        and (
          ...
          ((.labels // []) | map(select(startswith("pilot:reclaim-count:")) | ltrimstr("pilot:reclaim-count:") | select(test("^[0-9]+\\z")) | tonumber) |
            if length > 0
...
_emit_query_one() {
  ...
  _merged=$(echo "$_merged" | _filter_exec_manual | _reconcile_empty_description_signal "$_db" | _reconcile_text_veto_labels "$_db" | _filter_candidates | _filter_dispatch_gates | _filter_built | _filter_unblocked "$_db" | _filter_explicit_deps "$_db")
...
if [ "$(_quiet_hours_blocks)" = "1" ]; then
  ...
  log "Quiet-hours signal UNREADABLE (stale/corrupt ${QUIET_HOURS_LEVEL_FILE}) — fail-open, dispatch proceeding normally this sweep (ga-dxyvxr)."
fi
...
if [ "$SMALL_SLOTS" -eq "0" ] && [ "$BIG_SLOTS" -eq "0" ]; then
  ...
  # ga-3hhnyn: a bead can carry story:approved and still be permanently vetoed
  # (pilot:no-auto-dispatch, blocked-on:*, waiting-on:*, next-action:*) — it
  # would not dispatch even with free slots. The old log blamed "both lanes
  # full" unconditionally, conflating "queued" with "actually dispatchable"
  # (measured live: 8 waiting, 0 eligible, yet logged as a lane-capacity
  # problem — "represada por capacidade" and "parada por veto" are OPPOSITE
  # operational calls). ELIGIBLE_APPROVED reuses the SAME already-fetched
  # JSON via a local jq filter — still one bd call, so the "cheap count"
  # intent above still holds. Narrow veto criterion, verified against that
...
TIER2_JSON=$(bd -C "$GC_CITY" list --json \
  ...
  -n 0 \
  2>/dev/null || echo "[]")
TIER2_JSON=$(echo "$TIER2_JSON" | _filter_exec_manual | _reconcile_empty_description_signal "$GC_CITY" | _reconcile_text_veto_labels "$GC_CITY" | _filter_candidates | _filter_dispatch_gates | _filter_built)
...
if [ "$PILOT_CTX_READY_QUERIES" = "1" ]; then
  ...
  CTXREADY_JSON=$(echo "$CTXREADY_JSON" | _filter_exec_manual | _reconcile_empty_description_signal "$GC_CITY" | _reconcile_text_veto_labels "$GC_CITY" | _filter_candidates | _filter_dispatch_gates | _filter_built)
...
if [ "$PILOT_CTX_READY_RIG_QUERIES" = "1" ]; then
  ...
  while IFS=$'\t' read -r _rig_ctx_name _rig_ctx_path; do
    ...
    _rig_ctx_typed=$(echo "$_rig_ctx_typed" | _filter_exec_manual | _reconcile_empty_description_signal "$_rig_ctx_path" | _reconcile_text_veto_labels "$_rig_ctx_path" | _filter_candidates | _filter_dispatch_gates | _filter_built)
...
if [ "$PILOT_WA_RIG_APPROVED_QUERIES" = "1" ]; then
  if [ -n "${PILOT_WA_RIG_TIER2_OVERRIDE+x}" ]; then
    ...
    WA_RIG_TIER2_JSON=$(echo "$PILOT_WA_RIG_TIER2_OVERRIDE" \
      | _filter_exec_manual | _reconcile_empty_description_signal "${GC_CITY}" | _reconcile_text_veto_labels "${GC_CITY}" | _filter_candidates | _filter_dispatch_gates | _filter_built \
    ...
    while IFS=$'\t' read -r _wa_rig2_name _wa_rig2_path; do
      ...
      _wa_rig2_filtered=$(echo "$_wa_rig2_raw" \
        | _filter_exec_manual | _reconcile_empty_description_signal "$_wa_rig2_path" | _reconcile_text_veto_labels "$_wa_rig2_path" | _filter_candidates | _filter_dispatch_gates | _filter_built \
  ...
  if [ -n "${PILOT_RIG_TIER1_OVERRIDE+x}" ]; then
    ...
    while IFS=$'\t' read -r _rt1_name _rt1_path; do
      ...
      _rt1_bugs=$(bd -C "$_rt1_path" list --json -t bug \
        ...
        --exclude-type epic \
        -n 0 2>/dev/null || echo "[]")
      _rt1_bugs=$(echo "$_rt1_bugs" | _filter_exec_manual | _reconcile_empty_description_signal "$_rt1_path" | _reconcile_text_veto_labels "$_rt1_path" | _filter_candidates | _filter_dispatch_gates | _filter_built | _filter_unblocked "$_rt1_path" | _filter_explicit_deps "$_rt1_path")
      _rt1_debt=$(bd -C "$_rt1_path" list --json -l "tech-debt" \
        ...
        --exclude-type epic \
        -n 0 2>/dev/null || echo "[]")
      _rt1_debt=$(echo "$_rt1_debt" | _filter_exec_manual | _reconcile_empty_description_signal "$_rt1_path" | _reconcile_text_veto_labels "$_rt1_path" | _filter_candidates | _filter_dispatch_gates | _filter_built | _filter_unblocked "$_rt1_path" | _filter_explicit_deps "$_rt1_path")
      ...
      _rt1_chore=$(bd -C "$_rt1_path" list --json -t chore \
        ...
        --exclude-type epic \
        -n 0 2>/dev/null || echo "[]")
      _rt1_chore=$(echo "$_rt1_chore" | _filter_exec_manual | _reconcile_empty_description_signal "$_rt1_path" | _reconcile_text_veto_labels "$_rt1_path" | _filter_candidates | _filter_dispatch_gates | _filter_built | _filter_unblocked "$_rt1_path" | _filter_explicit_deps "$_rt1_path")
      _rt1_task=$(bd -C "$_rt1_path" list --json -t task \
        ...
        --exclude-type epic \
        -n 0 2>/dev/null || echo "[]")
      _rt1_task=$(echo "$_rt1_task" | _filter_exec_manual | _reconcile_empty_description_signal "$_rt1_path" | _reconcile_text_veto_labels "$_rt1_path" | _filter_candidates | _filter_dispatch_gates | _filter_built | _filter_unblocked "$_rt1_path" | _filter_explicit_deps "$_rt1_path")
  ...
  if [ "$DRY_RUN" = "1" ]; then
    ...
    if [ "$_DISPATCH_REUSE" = "1" ]; then
      timeout 15 gc --city "$GC_CITY" session submit "$_DISPATCH_SESS_REF" "$DISPATCH_TASK" --intent follow_up \
        2>/dev/null \
        || { warn "Could not submit to $_DISPATCH_SESS_REF — un-suppressing sling (ga-h6trx3) so it stays bd-ready-visible instead of relying on the next hook cycle"; _pilot_unsuppress_sling "$GC_CITY" "$SLING_BEAD_ID"; }
    else
      timeout 15 gc --city "$GC_CITY" session nudge "$_SLING_TARGET" "$DISPATCH_TASK" \
        2>/dev/null \
        || { warn "Could not nudge $_SLING_TARGET — un-suppressing sling (ga-h6trx3) so it stays bd-ready-visible instead of relying on the next hook cycle"; _pilot_unsuppress_sling "$GC_CITY" "$SLING_BEAD_ID"; }
    fi
