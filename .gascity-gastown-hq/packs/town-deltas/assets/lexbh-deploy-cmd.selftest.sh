#!/usr/bin/env bash
# Selftest for the lexbh rig's deploy_cmd (ga-apcolz).
#
# Extracts the EXACT deploy_cmd string configured in delivery-runbooks.toml
# (via the same regex get_runbook_field() in story-delivery.sh uses) and runs
# it — with /Users/athos/.lexbh substituted for a disposable temp clone — to
# prove the three properties the bead's acceptance criteria require:
...
# Never touches the real /Users/athos/.lexbh — everything runs against throwaway
# git repos under a mktemp -d directory.
set -u
...
for block in blocks:
    ...
    if m and m.group(1) == "lexbh":
        fm = re.search(r'deploy_cmd\s*=\s*"([^"]*)"', block)
        print(fm.group(1) if fm else "")
        sys.exit(0)
sys.exit(1)
PYEOF
) || { echo "FAIL: could not find a [[rig]] block named \"lexbh\" in $TOML"; exit 1; }

if [ -z "$RAW_CMD" ]; then
  echo "FAIL: lexbh deploy_cmd is empty in $TOML — nothing to test"
  exit 1
fi
case "$RAW_CMD" in
  *"/Users/athos/.lexbh"*) : ;;
  *) echo "FAIL: deploy_cmd does not reference /Users/athos/.lexbh — selftest cannot retarget it to a temp clone: $RAW_CMD"; exit 1 ;;
esac

pass=0; fail=0
must() { "$@" || { echo "SETUP FAILED: $*" >&2; exit 1; }; }  # unrecoverable — abort, don't cascade into confusing test failures
ok()  { pass=$((pass+1)); }
...
# -b main pins the bare repo's default branch explicitly — without it, clone
# advertises whatever init.defaultBranch happens to be on this machine (often
# not "main"), and every subsequent clone below prints a harmless but noisy
# "remote HEAD refers to nonexistent ref" warning since we only ever use "main".
...
# Clone a fresh "runtime dir" caught up with origin's CURRENT tip.
seed_runtime() { # $1 = target dir
  must git clone --quiet "$ORIGIN" "$1"
  must git -C "$1" checkout --quiet -B main
}

advance_origin() { # adds one more commit to origin/main via $OTHER
  must git -C "$OTHER" -c user.name=test -c user.email=test@test commit --quiet --allow-empty -m "upstream work $RANDOM"
  must git -C "$OTHER" push --quiet origin HEAD:main
}

run_deploy() { # $1 = runtime dir; substitutes it for /Users/athos/.lexbh in the real command and evals
  local rt="$1"
  local cmd="${RAW_CMD//\/Users\/athos\/.lexbh/$rt}"
...
echo "lexbh-deploy-cmd selftest: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
