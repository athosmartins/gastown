...
# marker_active_from_labels <labels-string> — pure text extraction (ga-usdm6p).
# Is a quality-gate-marker's gate-status still ACTIVE (mid-flight, not
# terminal)? The canonical active set — ready/queued/claimed/dispatching/
# running — is already established elsewhere in THIS file: gap2_query_active_
# markers' own --label-any list (above) queries markers on exactly those five
# values, and the ga-4tgga docstring on classify_gap2_bugtask_verdict spells
# it out directly ("gate-status ready/queued/claimed/dispatching/running is
# mid review, not orphaned"). Vector B's own inline check used to test only
# queued|claimed|dispatching — silently missing `ready` and `running` — so a
# marker sitting at either of those two states read as terminal/gone and had
# its live companion gate-run wrongly superseded (ga-usdm6p live incident:
# marker ga-c683w6 sat gate-status:ready with a genuinely live companion
# review; Vector B closed the gate-run AND cascade-closed an in-progress
# reviewer verdict citing "marker terminal/gone" — false, the marker was
# never closed, no such log entry exists anywhere).
# A marker with NO gate-status label at all is the pre-existing fail-safe:
# unrecognized/absent reads as active, never as terminal, so a labeling race
...
# is_newest_alive_in_group <this_id> <this_created> <siblings_tsv> <closed_ids> — pure (ga-usdm6p).
# siblings_tsv: newline-separated "<id>\t<created_at>" for every OTHER member of
#   this gate-run's dedup key-group (this_id itself excluded by the caller).
# closed_ids: space-separated ids already superseded/closed EARLIER in the same
#   sweep's Step 0b loop. Keep-newest dedup (dedup_gaterun_action above) only
#   ever supersedes a run when some sibling is genuinely still alive AND newer
#   — but the loop's key/created/id arrays are a single snapshot fetched once
#   at sweep start (the shared prelude) and never updated as the loop's own
#   closes land. Two gate-run beads sharing a key, each superseded via a
#   DIFFERENT vector within the SAME sweep (one via supersede:marker, the next
#   via keep-newest dedup 12s later) is exactly what excluding closed_ids here
#   prevents: without it, the second closure cites "a newer running run
#   exists," naming a sibling THIS SAME sweep already closed — a false,
#   misleading reason that cost real investigation time chasing a
#   contradiction that never existed in the data, only in the stale read
#   (ga-usdm6p live incident: ga-j8bc62 closed at 22:27:45, then ga-ibema9
#   closed 12s later at 22:27:57 citing ga-j8bc62 as "a newer running run").
# Echoes 1 iff no OTHER, not-yet-closed-this-sweep sibling is strictly newer
# (by created_at, ties broken lexically by id — same rule the caller already
# used pre-fix). Malformed siblings_tsv rows (missing id) are skipped, never
# guessed.
is_newest_alive_in_group() {
  local this_id="$1" this_created="$2" siblings_tsv="$3" closed_ids="$4"
  local id created
  while IFS=$'\t' read -r id created; do
    [ -z "$id" ] && continue
    case " $closed_ids " in *" $id "*) continue ;; esac
    if [[ "$created" > "$this_created" ]] || { [ "$created" = "$this_created" ] && [[ "$id" > "$this_id" ]]; }; then
      echo 0; return
    fi
  done <<< "$siblings_tsv"
  echo 1
...
# gate_bash32_verdict <changed> <checked> <failed> [<unmeasured>]
...
#   <unmeasured> (gate-fix 1): the changed-file LIST itself could not be read
#   (git diff failed, or its scratch file could not be created) — then
#   `changed` is not "0 files", it is UNKNOWN, and must never reach
#   sem-sh-alterado. Only the exact value 0 means "the list was read"; an
#   omitted argument is the legacy 3-arg call (== 0), but an explicitly empty
#   or non-zero one is treated as unmeasured.
#     sem-sh-alterado    — changed=0 (confirmed): the branch touches no .sh
#                           file under either protected tree. Never blocks.
#     nao-consegui-medir — the file list was unreadable, or changed>0 but
#                           checked!=changed (worktree/git-show extraction
#                           failed for at least one file, or /bin/bash itself
#                           was unusable) — an INCOMPLETE measurement is never
#                           treated as proof of either outcome. Never blocks
#                           (fail-open on uncertainty, same posture as every
#                           other submission-time check in this file) — but
#                           the call site LOGS it, so it is never silent.
#     bash32-ok          — changed>0, every file measured, ALL parse clean
#                           under /bin/bash -n. Never blocks.
#     bash32-fail        — AT LEAST ONE file was measured and fails to parse
#                           under /bin/bash -n. BLOCKS — a CONFIRMED failure
#                           is proof by itself, even if some other file could
#                           not be measured (that file is re-measured on the
#                           resubmission). This is exactly the ga-6aj348
...
#   and passes in only these counts.
gate_bash32_verdict() {
  local changed="$1" checked="$2" failed="$3" unmeasured="${4-0}"
  case "$changed" in
    ''|*[!0-9]*) printf 'nao-consegui-medir'; return 0 ;;
  esac
  case "$checked" in
...
# (ga-kgtiw/ga-7fwt1).
#
# ga-0m6tgc: MOVED here from quality-gate-dispatcher.sh (originally
# extracted from gate_bead_active_sibling_branch there, ga-lxz5w). Placed
# BEFORE the GATE_GUARD_LIB_ONLY cutoff below — same reasoning as
# branch_bead_commit_verdict/gate_ab_arm_for_bead above (ga-pj5va): a
# function defined past that cutoff is invisible to any lib-only sourcing,
# including story-delivery.sh's own `GATE_GUARD_LIB_ONLY=1 source` of this
# file, which is the entire reason for the move — story-delivery.sh's Step 1
# needs this exact marker-walk to hold a story bead with a still-open
# sibling gate marker (source-bead:) instead of deploying on the first
# marker to reach PASS. quality-gate-dispatcher.sh keeps calling it exactly
# as before via its own (unconditional, already-existing) source of this
# file — gate_bead_active_sibling_branch and gate_bead_terminal_failed_
# sibling_branch there are unchanged, only this shared building-block moved.
#
...
# that. See gate_bead_terminal_failed_sibling_branch (quality-gate-
# dispatcher.sh) for that use.
#
...
# quality-gate-dispatcher.sh L2117-2126's warning against this same mix-up
# in a different function). Both consumers of this data in quality-gate-
# dispatcher.sh (gate_pick_active_sibling, gate_pick_terminal_failed_
# sibling) read 3 tab-separated fields per line — a 2-field reader would
...
gate_bead_sibling_status_lines() {
  ...
  for i in $(seq 0 $((count - 1))); do
    ...
    # future interruption) can briefly carry TWO gate-status:* labels; blind
    # `head -1` would pick one arbitrarily and could feed
    # gate_pick_active_sibling/gate_bead_live_merge_block a WRONG status,
    # hiding or fabricating a genuinely concurrent sibling during a merge
    # decision. marker_status_from_labels() returns the value IFF exactly
    # one match exists, else empty — 0 and 2+ both collapse to the same safe
    # "unknown" outcome handled below, never a guess.
    ...
    if [ -z "$status" ]; then
      ...
      printf '[%s] [quality-gate-guard] ALERT: sibling marker %s (bead %s, branch %s) is OPEN with %s — ga-kgtiw/ga-7fwt1 invisible-or-ambiguous-marker signature. Excluded from sibling-race detection; needs manual gate-status repair.\n' \
...
if [ "$GATE_RUN_COUNT" -gt 0 ]; then
  ...
  # Ids this sweep's own loop has already superseded/closed (ga-usdm6p). Fed to
  # is_newest_alive_in_group below so a later iteration's dedup check never
  # cites an earlier iteration's OWN closure as "a newer running sibling" —
  # see that function's docstring for the live incident this prevents.
  CLOSED_THIS_SWEEP_IDS=""
  ...
  for i in $(seq 0 $((GATE_RUN_COUNT - 1))); do
    ...
    # ── Dedup decision: does a NEWER, still-alive running gate-run share this
    # key? Keep-newest never supersedes the newest run, so a live re-dispatch
    # (always the newest) is safe; only the stale older siblings (guard
    # tracking bead, dead-dispatcher re-queue remnants) are superseded.
    # "Still-alive" matters: GR_KEY_ARR/GR_CREATED_ARR/GR_ID_ARR are a single
    # snapshot fetched once at sweep start (shared prelude), so without
    # excluding CLOSED_THIS_SWEEP_IDS a sibling THIS SAME sweep already closed
    # in an earlier iteration would still count as "newer" here — see
    # is_newest_alive_in_group's docstring for the live incident (ga-usdm6p).
    _key="${GR_KEY_ARR[$i]}"
    _group_count=0; _siblings_tsv=""
    if [ -n "$_key" ]; then
      for j in $(seq 0 $((GATE_RUN_COUNT - 1))); do
        [ "${GR_KEY_ARR[$j]}" = "$_key" ] || continue
        _group_count=$((_group_count + 1))
        [ "$j" -eq "$i" ] && continue
        _siblings_tsv="${_siblings_tsv}${GR_ID_ARR[$j]}	${GR_CREATED_ARR[$j]}
"
      done
    fi
    _is_newest=$(is_newest_alive_in_group "$GR_ID" "${GR_CREATED_ARR[$i]}" "$_siblings_tsv" "$CLOSED_THIS_SWEEP_IDS")
    ...
    if [ -n "$RIGSCAN_GAP1_IDS" ]; then
      ...
      for RIGSCAN_OI_ID in $RIGSCAN_GAP1_IDS; do
        ...
        for RIGSCAN_PAT in "refs/heads/fix/${RIGSCAN_OI_ID}" "refs/heads/fix/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/feat/${RIGSCAN_OI_ID}" "refs/heads/feat/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/feature/${RIGSCAN_OI_ID}" "refs/heads/feature/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/refactor/${RIGSCAN_OI_ID}" "refs/heads/refactor/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/docs/${RIGSCAN_OI_ID}" "refs/heads/docs/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/chore/${RIGSCAN_OI_ID}" "refs/heads/chore/${RIGSCAN_OI_ID}-*" \
                   "refs/heads/test/${RIGSCAN_OI_ID}" "refs/heads/test/${RIGSCAN_OI_ID}-*" \
...
if [ "$INFLIGHT_COUNT" -gt 0 ]; then
  ...
  if [ -n "$GAP1_IDS" ]; then
    ...
    for OI_ID in $GAP1_IDS; do
      ...
      # Find branch tip by convention: fix/<id>*, feat/<id>*, feature/<id>*,
      # refactor/<id>*, docs/<id>*, chore/<id>*, test/<id>*, or crew/<agent>/<id>*
      # Prefer ls-remote (live) with for-each-ref as local cache fallback.
      OI_BRANCH_SHA=""
      for PAT in "refs/heads/fix/${OI_ID}" "refs/heads/fix/${OI_ID}-*" \
                 "refs/heads/feat/${OI_ID}" "refs/heads/feat/${OI_ID}-*" \
                 "refs/heads/feature/${OI_ID}" "refs/heads/feature/${OI_ID}-*" \
                 "refs/heads/refactor/${OI_ID}" "refs/heads/refactor/${OI_ID}-*" \
                 "refs/heads/docs/${OI_ID}" "refs/heads/docs/${OI_ID}-*" \
                 "refs/heads/chore/${OI_ID}" "refs/heads/chore/${OI_ID}-*" \
                 "refs/heads/test/${OI_ID}" "refs/heads/test/${OI_ID}-*" \
          ...
          if [ -n "$GAP2_MAIN_SHA" ]; then
            for GAP2_TRY_ID in "$SC_ID" "$SLING_ID"; do
              ...
              for GAP2_PAT in "refs/heads/fix/${GAP2_TRY_ID}" "refs/heads/fix/${GAP2_TRY_ID}-*" \
                         "refs/heads/feat/${GAP2_TRY_ID}" "refs/heads/feat/${GAP2_TRY_ID}-*" \
                         "refs/heads/feature/${GAP2_TRY_ID}" "refs/heads/feature/${GAP2_TRY_ID}-*" \
                         "refs/heads/refactor/${GAP2_TRY_ID}" "refs/heads/refactor/${GAP2_TRY_ID}-*" \
                         "refs/heads/docs/${GAP2_TRY_ID}" "refs/heads/docs/${GAP2_TRY_ID}-*" \
                         "refs/heads/chore/${GAP2_TRY_ID}" "refs/heads/chore/${GAP2_TRY_ID}-*" \
                         "refs/heads/test/${GAP2_TRY_ID}" "refs/heads/test/${GAP2_TRY_ID}-*" \
...
# limitation. MEASURED root cause (re-measured on /bin/bash 3.2.57 in the
# ga-7dx2vw gate-fix pass, replacing an earlier wrong description): a `case`
# whose patterns are NOT parenthesized, sitting INSIDE a `$( ... )` command
# substitution — bash 3.2 rejects the first `;;` ("syntax error near
# unexpected token `;;'"). Where `esac` and the closing `)` fall is
# irrelevant: `esac` on its own line, or `)` on the next line, are rejected
# just the same. It parses when the patterns carry a LEADING paren —
# `$(case $x in (a) ...;; (*) ...;; esac)` — or when the case is moved out of
...
# file (with rename detection on, --diff-filter=AM skipped it entirely — a
# silent bypass). The file list is NUL-delimited (-z), so a non-ASCII or
# otherwise git-quoted path is read verbatim instead of being dropped.
#
...
if [ -n "$RIG_PATH" ] && [ -n "$BEAD_ID" ] && [ -n "$BRANCH" ]; then
  ...
  if ! git -C "$RIG_PATH" fetch origin main "$BRANCH" --quiet 2>/dev/null; then
    _B32_UNMEASURED=1
  fi
  ...
  if [ "$_B32_UNMEASURED" -eq 0 ] && [ -n "$_B32_MAIN_SHA" ] && [ -n "$_B32_BRANCH_SHA" ] && [ -x /bin/bash ]; then
    _B32_BASE=$(git -C "$RIG_PATH" merge-base "$_B32_BRANCH_SHA" "$_B32_MAIN_SHA" 2>/dev/null || echo "")
    if [ -n "$_B32_BASE" ]; then
      _B32_REFS=1
      _B32_LIST=$(mktemp "${TMPDIR:-/tmp}/gate-b32-list-XXXXXX" 2>/dev/null || echo "")
      _B32_WT=$(mktemp -d "${TMPDIR:-/tmp}/gate-b32-XXXXXX" 2>/dev/null || echo "")
      # The diff's OWN exit status decides whether the list was read — it is
      # redirected to a file, never piped through grep or `|| true`. -z: NUL-
      # delimited, so a non-ASCII / git-quoted path is read verbatim instead of
      # being emitted quoted (and then dropped by a `\.sh$` filter).
      # --no-renames: a renamed-and-edited file shows up as an ADD of its new
      # path and gets parsed; with rename detection on, AM skipped it.
      if [ -n "$_B32_LIST" ] && git -C "$RIG_PATH" diff -z --name-only --no-renames --diff-filter=AM "${_B32_BASE}..${_B32_BRANCH_SHA}" -- 'packs/town-deltas/assets' 'scripts' > "$_B32_LIST" 2>/dev/null; then
        while IFS= read -r -d '' _b32_f; do
          case "$_b32_f" in
            *.sh) ;;
            *) continue ;;
          esac
          ...
          if git -C "$RIG_PATH" show "${_B32_BRANCH_SHA}:$_b32_f" > "$_B32_WT/$_b32_f" 2>/dev/null; then
            ...
            if [ "$_b32_class" = "ok" ]; then
              ...
              # bash -n already prefixes its message with the file name; only
              # add ours when it did not (avoids "path: path: line N: ...").
              case "$_b32_err" in
                "$_b32_f"*) _b32_line="$_b32_err" ;;
                *) _b32_line="$_b32_f: $_b32_err" ;;
              esac
