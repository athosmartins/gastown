...
# `set -euo pipefail` (L54) — a POSIX special builtin dies immediately on a
...
_AUTO_REFINO_QHC_SIB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/quiet-hours-check.sh"
if [ -r "$_AUTO_REFINO_QHC_SIB" ]; then
  source "$_AUTO_REFINO_QHC_SIB"
fi
unset _AUTO_REFINO_QHC_SIB
...
_ar_lock_hb_age() {
  local _path="${1:-$AUTO_REFINO_LOCK_HB}"
  local _mt _now
  _now=$(date +%s)
...
_acquire_ar_lock() {
  ...
  # renaming. If a peer wins the SAME race first and recreates a fresh lock at
  # $AUTO_REFINO_LOCK_DIR before we get here, our mv will happily rename THEIR
  # live lock away instead of the original zombie (ga-bong5, same class as
  # ga-byd3u: a slow recoverer can steal a peer's freshly-recreated lock, a
  # genuine double-acquisition — two concurrent sweeps both believing they
  # hold the lock). So re-validate staleness on the copy we now exclusively
  # own (nobody else can touch this uniquely-tokened path) before trusting
  # the mv actually caught what we think it did.
  local _reaped="${AUTO_REFINO_LOCK_DIR}.reaping.${AUTO_REFINO_LOCK_TOKEN}"
  if mv "$AUTO_REFINO_LOCK_DIR" "$_reaped" 2>/dev/null; then
    local _reaped_age
    _reaped_age=$(_ar_lock_hb_age "$_reaped/heartbeat")
    if [ "$_reaped_age" -lt "$AUTO_REFINO_LOCK_MAX_AGE" ]; then
      ...
      mv "$_reaped" "$AUTO_REFINO_LOCK_DIR" 2>/dev/null
