...
# other bd failure. Duplicated identically in quality-gate-guard.sh (no
# shared-lib chokepoint between the two files — same tradeoff this
# function's own neighbor default_pool_route_for_rig already documents
# above).
...
# gate_release_stale_assignee <bead_id> <merge_verified> [<city>]
...
# git-state heuristic — then retry the plain assign once more. Deliberately
# defined BEFORE the GATE_DISPATCHER_LIB_ONLY early-return (same reasoning as
...
#
# <merge_verified> is a REQUIRED literal "1", not optional sugar — it is the
# caller's attestation that it has ALREADY confirmed, via git (merge-base/
# patch-id — NOT this function, NOT `bd reclaim`), that the branch behind
# this claim is merged. Anything else (missing, blank, "0") refuses outright
# and returns 1 WITHOUT calling bd at all — a future call site that forgets
# this precondition fails loudly instead of silently getting the weaker,
# pre-ga-2emo8 behavior. Every one of this function's call sites must
# establish that BEFORE calling (see each call site's own comment).
#
# ga-2emo8 (supersedes the pre-ga-z93p0 caveat this comment used to carry —
# see git history for the original text): `bd reclaim --older-than 0s`
# reclaims a lease ONLY if it has ALREADY EXPIRED. Before ga-z93p0, heartbeat
# was fake (every formula's "heartbeat" step called `gc bd heartbeat`, an
# unrelated dashboard-only stamp bd's own lease code never reads), so nearly
# any claim looked stale within ~5min regardless of the worker's actual
# state — reclaim-then-retry alone was enough. ga-z93p0 wired the REAL `bd
# heartbeat` into mol-do-work.toml, called immediately before `git push` —
# so a claim is now routinely still FRESH the moment this function runs,
# seconds after its own branch merged. "Lease fresh" and "no work remains
# behind this claim" are now two DIFFERENT questions, and reclaim only ever
# answers the first (live-reproduced on ga-7mbry: both WARNs below fired
# verbatim against a confirmed-merged, gate-PASSED source bead). This
# function's authority to override a still-live claim comes from
# <merge_verified> — the caller's independent, stronger signal — never from
# reclaim succeeding. When the plain assign AND the reclaim-then-retry both
# still fail, escalate ONE more step: a bead-scoped `--force` (never blind —
# gated on merge_verified="1" — and never ad hoc: this is the only place in
# the file that does it, same centralization ga-v5acl already established
# for the weaker escalation). This satisfies, rather than contradicts,
# ga-v5acl's original concern about unconditional --force: the old site B
# forced with NO independent proof of anything; this forces only after the
# caller has independently proven the work is done.
#
# Returns 0 iff the bead ends up with no assignee; 1 otherwise (merge not
# verified, or bd erroring on every attempt including the forced one).
# SELFTEST-EXTRACT gate-release-stale-assignee-fn: BEGIN
gate_release_stale_assignee() {
  local _bead_id="$1" _merge_verified="$2" _city="${3:-$BEAD_CITY}"
  [ -z "$_bead_id" ] && return 1
  if [ "$_merge_verified" != "1" ]; then
    warn "gate_release_stale_assignee: refusing $_bead_id — called without merge_verified=1 (ga-2emo8 contract)."
    return 1
  fi
  ...
  warn "  Assignee clear on $_bead_id still refused after lease-aware reclaim — forcing (ga-2emo8: merge independently verified by caller)."
  bd -C "$_city" assign "$_bead_id" "" --force -q 2>/dev/null
}
# SELFTEST-EXTRACT gate-release-stale-assignee-fn: END

# gate_close_source_terminal <bead_id> <close_reason> <merge_verified> [<city>]
# ga-v5acl: shared close-with-lease-aware-retry primitive, sibling to
# gate_release_stale_assignee above (same escalation mechanism, applied to
# the CLOSE call instead of assign). Replaces the one site (the old
# PASS-path bug/task close, ga-esbg/wa-j824s) that used to escalate to
# `bd assign ... --force` + `bd close ... --force` UNCONDITIONALLY, without
# checking the claim was actually abandoned. Prints the bd close error once
# (via warn) on the first failed attempt, so the log names what happened
# before the reclaim-and-retry. Same LIB_ONLY-early / SELFTEST-EXTRACT
# placement rationale, same required <merge_verified> contract, and the same
# ga-2emo8 force-escalation as gate_release_stale_assignee above — see its
# header comment for the full reasoning (why reclaim alone stopped being
# enough post-ga-z93p0, and why a merge-verified-gated --force here doesn't
# reopen ga-v5acl's original concern). Only call this from a site that has
# independently verified the branch is already merged, same as this
# function's own 3 call sites (B/C/D).
#
# Returns 0 iff the bead ends up closed; 1 otherwise (merge not verified, or
# bd erroring on every attempt including the forced one — the caller's
# post-merge verification surfaces the residual re-pick vector).
# SELFTEST-EXTRACT gate-close-source-terminal-fn: BEGIN
gate_close_source_terminal() {
  local _bead_id="$1" _reason="$2" _merge_verified="$3" _city="${4:-$BEAD_CITY}"
  local _out _rc=0
  if [ "$_merge_verified" != "1" ]; then
    warn "gate_close_source_terminal: refusing $_bead_id — called without merge_verified=1 (ga-2emo8 contract)."
    return 1
  fi
  ...
  bd -C "$_city" reclaim --id "$_bead_id" --older-than 0s -q 2>/dev/null || true
  if bd -C "$_city" close "$_bead_id" -r "$_reason" 2>/dev/null; then
    return 0
  fi
  warn "  Close of $_bead_id still refused after lease-aware reclaim — forcing (ga-2emo8: merge independently verified by caller)."
  bd -C "$_city" close "$_bead_id" -r "$_reason" --force 2>/dev/null
}
...
# ── ga-a6etc2: bounded clean-retry — per-marker cooldown, hard cap, and a ─────
#    requeue write that respects a transition someone else made mid-sweep ───────
#
# INCIDENT (2026-09-25 09:44-11:06 local, 82 min): one marker (ga-g5s956, branch
# crew/wa-worker/wa-wqn2v) won 28+ consecutive sweeps while 23 others waited, and
# nothing alarmed. Three separate defects stacked, all three fixed here/below:
#
#  1. SELECTION: tier 1 of the marker-select block admits a has_rebase_fail
#     marker once its OWN exile passed GATE_EXILE_OVERDUE_SECONDS (ga-0ye7ar) —
#     a deliberate "an exiled marker must eventually get an attempt even in a
#     queue that never empties" guarantee. But nothing re-armed that clock after
#     the attempt FAILED, so the marker stayed in tier 1 (oldest-first, above
#     every healthy marker) on every following sweep. Measured: gate:exiled-since
#     07:40, overdue at 09:10, first HOL sweep 09:44, then every sweep to 11:06.
#     Fix: gate:retry-cooldown-until:<epoch> (below), stamped on each failed
#     attempt of the unbounded path; the selection block skips a marker until it
#     expires. gate:exiled-since is deliberately NOT touched — gate_exile_
#     watchdog_sweep (ga-faw5o) measures the exile's total age with it.
#  2. THE BOUND: ga-y5c29l made a proven-clean marker whose retries are exhausted
#     stay `queued` instead of circuit-breaking (correct for a transient
#     environment fault: wa-llq1a/wa-4zmm1) — but it removed the ONLY bound, so
#     gate:rebase-fail-count reached 32 with no exit. GATE_CLEAN_RETRY_HARD_CAP
#     restores one: at that many total failed attempts the marker parks at
#     gate-status:needs-rebase (the legacy escalation: mail Mayor + source-bead
#     label). Default 7 = the 3 ordinary attempts + 4 more spaced by the cooldown
#     (>= ~1h of coverage for an environment fault, but no longer unbounded).
#     A value <= MAX_REBASE_ATTEMPTS disables the extension (pre-y5c29l behaviour).
#  3. THE REVERT: the Mayor's manual queued->needs-rebase (11:03) was undone by
#     THIS dispatcher: set_gate_status strips EVERY gate-status:* label and writes
#     the target, so a transition made while the marker was `dispatching` (the
#     ~25s each sweep holds it) was erased by the sweep's own closing
#     set_gate_status(queued). Ruled out as the reverter: gate-recovery-watchdog
#     ("NEVER requeue"s a needs-rebase marker) and the guard's Vector A (it only
#     touches dispatching/claimed, and its only entry for this marker was logged
#     two minutes BEFORE the Mayor's flip). The mechanism is confirmed from the
#     code and the timing (Mayor comment 11:03, claim 11:03:37, sweep end ~11:04);
...
case "$GATE_CLEAN_RETRY_HARD_CAP" in ''|*[!0-9]*) GATE_CLEAN_RETRY_HARD_CAP=7 ;; esac

# gate_retry_cooldown_stamp <marker_id> [now_epoch]
# Writes gate:retry-cooldown-until:<now+GATE_RETRY_COOLDOWN_SECONDS> and removes
# any older cooldown label it can SEE. ADD before REMOVE (same interrupted-
# transition invariant as set_gate_status/ga-i0n83). GATE_RETRY_COOLDOWN_SECONDS=0
# disables the stamp. The selection block reads the labels with `max`, so an
# older label that could not be listed or removed can never SHORTEN a cooldown,
# and — deadlines only grow — never extend one either.
# A failed label write is logged, not swallowed: the marker then stays eligible
...
# The closing write of a sweep that holds a marker at <expected_status>
...
# overwrite, so behaviour under a Dolt hiccup is exactly what it was before this
# function existed (a stranded `dispatching` marker would wait out the 30m Vector A
...
if [ "$OVERALL_VERDICT" = "PASS" ] && [ -n "$BEAD_ID" ] && [ -n "$BRANCH" ]; then
  ...
  if [ -n "$GATE_LXZ5W_SIBLING" ]; then
    ...
    # ga-36ta4 (ga-55syh follow-up): apply-then-VERIFY, not apply-then-assume —
    # see gate_apply_needs_human()'s own header for the full incident.
    _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:sibling-race")
    [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-lxz5w/ga-36ta4) — circuit-breaker NOT armed."
    bd -C "$BEAD_CITY" comment "$BEAD_ID" "Dispatcher (ga-lxz5w): held $BRANCH — a sibling branch ($GATE_LXZ5W_SIB_BRANCH, gate-status:$GATE_LXZ5W_SIB_STATUS) is concurrently active in the gate for this same bead. Two branches racing to resolve the same bead cannot be auto-merged without reconciliation. $(gate_needs_human_clause "$_NH_STATUS") A human must pick the correct branch (or merge the superset) and clear gate:needs-human before either can proceed." 2>/dev/null || true
    gc --city "$GC_CITY" mail send mayor \
      -s "Gate needs-human: 2 branches racing on $BEAD_ID (ga-lxz5w)" \
      -m "$(printf 'Source bead %s has 2+ branches concurrently active in the quality gate.\n\nThis branch %s just PASSED review, but sibling branch %s is still %s — auto-merging either would silently discard the other. %s\n\nA human must reconcile: pick the correct/superset branch, then clear gate:needs-human.\n\nBead: %s   Rig: %s\nThis branch: %s (gate run %s)\nSibling branch: %s (gate-status:%s)' \
        "$BEAD_ID" "$BRANCH" "$GATE_LXZ5W_SIB_BRANCH" "$GATE_LXZ5W_SIB_STATUS" "$(gate_needs_human_clause "$_NH_STATUS")" "$BEAD_ID" "$RIG" "$BRANCH" "$GATE_RUN_ID" "$GATE_LXZ5W_SIB_BRANCH" "$GATE_LXZ5W_SIB_STATUS")" \
    ...
    notify_author_with_fallback "$BEAD_ID" "$NOTIFY_AUTHOR" "$AUTHOR" \
      ...
      "$(printf 'Your branch %s PASSED gate review, but it was NOT merged.\n\nA sibling branch %s is concurrently active in the gate for the same source bead %s (gate-status:%s). Auto-merging either branch would silently discard the other. %s\n\nNothing to do from your side right now — a human must pick the correct/superset branch and clear gate:needs-human before either can proceed.\n\nBead: %s   Rig: %s\nYour branch: %s (gate run %s)\nSibling branch: %s (gate-status:%s)' \
        "$BRANCH" "$GATE_LXZ5W_SIB_BRANCH" "$BEAD_ID" "$GATE_LXZ5W_SIB_STATUS" "$(gate_needs_human_clause "$_NH_STATUS")" "$BEAD_ID" "$RIG" "$BRANCH" "$GATE_RUN_ID" "$GATE_LXZ5W_SIB_BRANCH" "$GATE_LXZ5W_SIB_STATUS")" \
      "sibling-branch race on $BEAD_ID (ga-lxz5w)"
    warn "ga-lxz5w: sibling branch $GATE_LXZ5W_SIB_BRANCH (gate-status:$GATE_LXZ5W_SIB_STATUS) active for bead $BEAD_ID — downgrading $BRANCH's PASS to FAIL (needs-human armed=$_NH_STATUS)."
  fi
fi
...
if [ "$OVERALL_VERDICT" = "PASS" ] && [ -n "$BEAD_ID" ] && [ -n "$BRANCH_SHA" ]; then
  ...
  if [ -n "$GATE_Y9A1D_BASE" ]; then
    ...
    if [ "$GATE_Y9A1D_VERDICT" = "no" ]; then
      ...
      _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:branch-content-mismatch")
      [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-y9a1d/ga-36ta4) — circuit-breaker NOT armed."
      ...
      gc --city "$GC_CITY" mail send mayor \
        ...
        -m "$(printf 'Branch %s (rig %s) reached the gate merge step for bead %s, but its own commits do not reference that bead at all.\n\n%s\n\nThis is the exact incident ga-y9a1d described: a branch ref silently reused by unrelated work, with the branch name intact and nothing else failing. %s\n\nBead: %s   Rig: %s\nBranch: %s (sha %s, gate run %s)\nBase (merge-base with %s): %s' \
          "$BRANCH" "$RIG" "$BEAD_ID" "$FAIL_REASONS" "$(gate_needs_human_clause "$_NH_STATUS")" "$BEAD_ID" "$RIG" "$BRANCH" "$BRANCH_SHA" "$GATE_RUN_ID" "$DEFAULT_BRANCH" "$GATE_Y9A1D_BASE")" \
...
# reviewable one.
  ...
  if [ "$OVERALL_VERDICT" = "PASS" ]; then
    ...
    if [ -n "$BEAD_ID" ] && [ "$DRY_RUN" != "1" ]; then
      ...
      # ga-v5acl: escalates via lease-aware `bd reclaim`, then (ga-2emo8) a
      # merge-verified --force, when the plain assign is refused — see
      # gate_release_stale_assignee. merge_verified=1: this PASS path only
      # reaches here after a confirmed successful merge (MERGE_SHA set above
      ...
      if [ "$IS_STORY" = "1" ]; then
        ...
        # ga-pm93k: the list-structure signal above is a HEURISTIC, not a
        # verified count of deliverables — an item that opens with an
        # alternative conjunction ("OU"/"ou alternativamente"/"OR") is an
        # ALTERNATIVE to the item before it, not an additional deliverable,
        # and this guard does not parse conjunctions. Real incident:
        # ga-sfj3i.1 was held with this exact miscount as the stated reason,
        # which misdirected the reviewer toward verifying "3 deliverables"
        # when the real scope was 2 — a wrong reason is not neutral, it
        # steers the check. Rather than add yet another per-token parser to
        # an already-repeatedly-patched heuristic (v1..v4: ga-k2wjn,
        # ga-zhfk8, ga-1yxyt, ga-cjrxh), the message stops ASSERTING the
        # count as fact and says what it actually is: a structural signal
        # worth a human glance. The hold itself is unchanged — still holds,
        # just says so honestly (ga-pm93k AC2/AC3).
        SCOPE_HOLD_WEAK_SIGNAL_NOTE="this bead body CONTAINS a numbered/lettered list structure (>=3 consecutive items — see detected lines below) that MAY enumerate multiple approved deliverables. This is a structural signal, not a verified count: an item starting with an alternative conjunction (OU, ou alternativamente, OR) is an ALTERNATIVE to the item before it, not an extra deliverable, so the real scope can be smaller than the item count suggests — read the detected lines before assuming N separate deliverables."
        # ga-pm93k AC1: the check that actually discriminates "gate passed"
        # from "is ready", independent of WHY this hold fired. ga-sfj3i.1
        # was almost released on this heuristic's word alone; only an
...
$PARTIAL_EVIDENCE" 2>/dev/null || true
        gc --city "$GC_CITY" mail send mayor \
          ...
          -m "$(printf 'Source bead %s PASSED the quality gate and merged (branch %s, sha %s, gate_run %s) but was NOT closed.\n\nga-k2wjn/ga-zhfk8: %s\n\n%s\n\n%s\n\nLabeled delivery:partial + scope:needs-review; Pilot will not re-dispatch it.\n\nReview the diff against the full enumerated scope: if complete, add label scope_covered:all and close manually (or re-submit to the gate); if partial, the remaining items are still live on this bead.\n\nBead: %s   Rig: %s\nBranch: %s (gate run %s, sha %s)' \
            "$BEAD_ID" "$BRANCH" "$MERGE_SHA" "$GATE_RUN_ID" "$SCOPE_HOLD_WEAK_SIGNAL_NOTE" "$PARTIAL_EVIDENCE" "$SCOPE_HOLD_ALWAYS_CHECK" "$BEAD_ID" "$RIG" "$BRANCH" "$GATE_RUN_ID" "$MERGE_SHA")" \
    ...
    if [ -n "$BEAD_ID" ]; then
      ...
      case "$NR_BD_STATUS" in
        ...
        *)
          ...
          # lease-aware escalation (bd reclaim --older-than 0s, then
          # ga-2emo8's merge-verified --force) as the other sites, and
          # actually surfaces a failure instead of eating it. merge_verified=1:
          # this branch only runs after gate_branch_already_merged confirmed
...
# must end the sweep cleanly here, never fall through to "claim marker null".
MARKER_ID=$(printf '%s\n' "$MARKER" | jq -r '.id // empty')
if [ -z "$MARKER_ID" ]; then
  # Do not ASSERT the cause, confirm it: count the markers that really carry an
  # unexpired cooldown, independently of the selection program above. If that
  # count is not the whole queue, the selection came back empty for a reason
  # this branch does not understand — say so, claim nothing (the inert choice),
  # and let a human see the mismatch in the log instead of a comforting story.
  _CD_N=$(printf '%s\n' "$MARKERS_JSON" | jq --argjson now "$GATE_MARKER_NOW_EPOCH" \
    '[.[] | select(([(.labels // [])[] | select(test("^gate:retry-cooldown-until:[0-9]+$")) | (sub("^gate:retry-cooldown-until:";"") | tonumber)] | (max // 0)) > $now)] | length' 2>/dev/null || echo "?")
  if [ "$_CD_N" = "$COUNT" ]; then
    log "All $COUNT queued marker(s) are inside their retry cooldown (gate:retry-cooldown-until, ga-a6etc2) — nothing eligible to claim this sweep; the earliest cooldown expiry re-admits it."
  else
    warn "Marker selection came back EMPTY with $COUNT queued marker(s), but only ${_CD_N:-?} of them carry an unexpired retry cooldown — cause UNKNOWN; claiming nothing this sweep (ga-a6etc2)."
  fi
  exit 0
fi
...
if [ -z "$BRANCH_SHA" ]; then
  ...
  if [ "$_NB_CLASS" = "rescuable" ]; then
    ...
    if [ -n "$BEAD_ID" ]; then
      ...
      _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "")
      [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-tgwq/ga-36ta4) — circuit-breaker NOT armed."
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "story:in-flight"  -q 2>/dev/null || true
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "gate:reviewing"   -q 2>/dev/null || true
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "pilot:dispatched" -q 2>/dev/null || true
      bd -C "$BEAD_CITY" assign       "$BEAD_ID" ""                 -q 2>/dev/null || true
      bd -C "$BEAD_CITY" comment "$BEAD_ID" "ga-tgwq: gate found branch '$BRANCH' absent from origin with UNCOMMITTED changes in a local worktree — the session likely died mid-edit. $(gate_needs_human_clause "$_NH_STATUS") A human or Mayor must inspect the pending diff before deciding whether to commit+push or discard. story:in-flight + pilot:dispatched stripped (Pilot lane slot freed)." 2>/dev/null || true
    fi
    gc --city "$GC_CITY" mail send mayor \
    ...
    if [ -n "$AUTHOR" ]; then
      gc --city "$GC_CITY" mail send "$AUTHOR" \
        ...
        -m "Your branch $BRANCH (bead $BEAD_ID) is absent from origin, and its local worktree has UNCOMMITTED changes — looks like your session died mid-edit. $(gate_needs_human_clause "$_NH_STATUS") Please inspect the local worktree and decide: commit+push, or discard, before resubmitting." \
        2>/dev/null || warn "Could not mail author $AUTHOR for dirty-local branch $BRANCH"
    fi
    log "ga-tgwq: branch $BRANCH absent from origin with dirty local worktree — marker $MARKER_ID at gate-status:error, bead $BEAD_ID needs-human armed=$_NH_STATUS, Mayor + author mailed."
    exit 0
  fi
  ...
  if [ "$_ACB" != "ok" ]; then
    ...
    # ga-7fwt1: set_gate_status() — add-before-remove, queried live.
    set_gate_status "$MARKER_ID" "error"
    # ga-36ta4 (ga-55syh follow-up): apply-then-verify FIRST, so the marker
    # comment right below (and everything after it) can cite the VERIFIED
    # state instead of asserting it before the write even happened.
    _NH_STATUS="failed"
    [ -n "$BEAD_ID" ] && _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:technical")
    [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-acb/ga-36ta4) — circuit-breaker NOT armed."
    bd -C "$GC_CITY" comment "$MARKER_ID" "ga-acb AUTO-CIRCUIT-BREAK (${_ACB}): branch '$BRANCH' is absent from origin. No reviewer can ever merge a non-existent branch. Marker permanently parked at gate-status:error. Source bead $BEAD_ID: $(gate_needs_human_clause "$_NH_STATUS")" 2>/dev/null || true
    if [ -n "$BEAD_ID" ]; then
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "story:in-flight"            -q 2>/dev/null || true
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "gate:reviewing"             -q 2>/dev/null || true  # wa-qq33j
      bd -C "$BEAD_CITY" label remove "$BEAD_ID" "pilot:dispatched"           -q 2>/dev/null || true
      bd -C "$BEAD_CITY" assign       "$BEAD_ID" ""                           -q 2>/dev/null || true
      bd -C "$BEAD_CITY" comment "$BEAD_ID" "ga-acb AUTO-CIRCUIT-BREAK: branch '$BRANCH' is absent from origin (marker $MARKER_ID). Gate cannot review or merge a non-existent branch. $(gate_needs_human_clause "$_NH_STATUS") story:in-flight + pilot:dispatched stripped (Pilot lane slot freed). gate:reviewing cleared (wa-qq33j)." 2>/dev/null || true
    fi
    gc --city "$GC_CITY" mail send mayor \
      -s "Gate circuit-break: $BRANCH absent from origin (${BEAD_ID:-unknown})" \
      -m "Branch $BRANCH (bead ${BEAD_ID:-unknown}, rig ${RIG:-unknown}, marker $MARKER_ID) is ABSENT from origin — gate cannot review or merge it. Auto-circuit-broken (ga-acb): marker permanently parked at gate-status:error. $(gate_needs_human_clause "$_NH_STATUS") Human or Mayor decision required: re-push the branch or close the bead." 2>/dev/null \
    ...
    if [ -n "$AUTHOR" ]; then
      gc --city "$GC_CITY" mail send "$AUTHOR" \
        ...
        -m "Your branch $BRANCH (bead $BEAD_ID) is ABSENT from origin — the gate cannot review or merge a branch that doesn't exist on the remote. $(gate_needs_human_clause "$_NH_STATUS") Re-push the branch (or ask the Mayor to re-anchor the work) before resubmitting." \
  ...
  if [ -n "$BEAD_ID" ]; then
    ...
    case "$(vb_status_action "$BD_STATUS")" in
      ...
      requeue)
        ...
        # escalation (bd reclaim --older-than 0s, then ga-2emo8's
        # merge-verified --force) — see gate_release_stale_assignee's header
        # comment above. merge_verified=1: this whole block is the
        # already-merged short-circuit (BRANCH confirmed already in
        # DEFAULT_BRANCH via git before this point — see vb_status_action
        # dispatch above).
        gate_release_stale_assignee "$BEAD_ID" 1 \
...
if [ "$BRANCH_IS_CURRENT" != "1" ]; then
  ...
  if [ "$BRANCH_IS_CURRENT" != "1" ]; then
    ...
    if [ "$_ACB_AHEAD" != "ok" ]; then
      ...
      # ga-36ta4 (ga-55syh follow-up): apply-then-verify FIRST (same reasoning
      # as the no_branch site above).
      _NH_STATUS="failed"
      [ -n "$BEAD_ID" ] && _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:technical")
      [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-acb ahead_dead/ga-36ta4) — circuit-breaker NOT armed."
      bd -C "$GC_CITY" comment "$MARKER_ID" "ga-acb AUTO-CIRCUIT-BREAK (${_ACB_AHEAD}): branch $BRANCH is ${REBASE_AHEAD:-?} commits ahead of main (> GATE_REBASE_AHEAD_MAX=${GATE_REBASE_AHEAD_MAX}) with no live author session (ga-it1of liveness check: ${REBASE_LIVENESS_TRACE:-none}). A server-side rebase of a large divergence with no one to resolve conflicts is futile. Marker permanently parked at gate-status:error. Source bead $BEAD_ID: $(gate_needs_human_clause "$_NH_STATUS")" 2>/dev/null || true
      if [ -n "$BEAD_ID" ]; then
        bd -C "$BEAD_CITY" label remove "$BEAD_ID" "story:in-flight"            -q 2>/dev/null || true
        bd -C "$BEAD_CITY" label remove "$BEAD_ID" "gate:reviewing"             -q 2>/dev/null || true  # wa-qq33j
        bd -C "$BEAD_CITY" label remove "$BEAD_ID" "pilot:dispatched"           -q 2>/dev/null || true
        bd -C "$BEAD_CITY" assign       "$BEAD_ID" ""                           -q 2>/dev/null || true
        bd -C "$BEAD_CITY" comment "$BEAD_ID" "ga-acb AUTO-CIRCUIT-BREAK (${_ACB_AHEAD}): branch $BRANCH is ${REBASE_AHEAD:-?} commits ahead (> ${GATE_REBASE_AHEAD_MAX} max) with no live author (marker $MARKER_ID). Liveness candidates checked (ga-it1of): ${REBASE_LIVENESS_TRACE:-none}. Large divergence + dead author = un-mergeable. $(gate_needs_human_clause "$_NH_STATUS") story:in-flight + gate:reviewing (wa-qq33j) + pilot:dispatched stripped." 2>/dev/null || true
      fi
      gc --city "$GC_CITY" mail send mayor \
        -s "Gate circuit-break: $BRANCH large divergence + dead author (${BEAD_ID:-unknown})" \
        -m "Branch $BRANCH (bead ${BEAD_ID:-unknown}, rig ${RIG:-unknown}, marker $MARKER_ID) is ${REBASE_AHEAD:-?} commits ahead of main (> ${GATE_REBASE_AHEAD_MAX} max) with no live author session. Liveness candidates checked (ga-it1of): ${REBASE_LIVENESS_TRACE:-none}. Auto-circuit-broken (ga-acb): marker parked at gate-status:error. $(gate_needs_human_clause "$_NH_STATUS") Human or Mayor must re-anchor the work or close the bead." 2>/dev/null \
      ...
      if [ -n "$AUTHOR" ]; then
        gc --city "$GC_CITY" mail send "$AUTHOR" \
          ...
          -m "Your branch $BRANCH (bead $BEAD_ID) is ${REBASE_AHEAD:-?} commits ahead of main (> ${GATE_REBASE_AHEAD_MAX} max) and your session was not live to resolve the rebase. $(gate_needs_human_clause "$_NH_STATUS") A human or the Mayor must re-anchor the work." \
      ...
      log "ga-acb circuit-break: $BRANCH ahead_dead — marker $MARKER_ID parked, bead $BEAD_ID needs-human armed=$_NH_STATUS."
    ...
    if [ "$_BEHIND_ACTION" = "circuit_break" ]; then
      ...
      if [ -n "$AUTHOR" ]; then
        gc --city "$GC_CITY" mail send "$AUTHOR" \
          ...
          -m "Branch $BRANCH (bead $BEAD_ID) base is ${REBASE_BEHIND:-?} commits behind current main (> ${GATE_REBASE_BEHIND_MAX} max) and no live author session was found to resolve the rebase. $(gate_needs_human_clause "$_NH_STATUS") A human or the Mayor must re-anchor the work." \
      ...
      log "ga-6dp9 circuit-break: $BRANCH behind_dead — marker $MARKER_ID parked, bead $BEAD_ID needs-human armed=$_NH_STATUS."
...
Action required: ${_REBASE_ACTION_ADVICE}." 2>/dev/null || true
      ...
      if [ "$NEXT_ATTEMPT" -lt "$MAX_REBASE_ATTEMPTS" ]; then
        ...
        if [ "$_ACB_RETRY" != "ok" ]; then
          ...
          _NH_STATUS="failed"
          [ -n "$BEAD_ID" ] && _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:technical")
          [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-acb retry_dead/ga-36ta4) — circuit-breaker NOT armed."
          bd -C "$GC_CITY" comment "$MARKER_ID" "ga-acb AUTO-CIRCUIT-BREAK (${_ACB_RETRY}): branch $BRANCH could not auto-rebase (${CONFLICT_FILES:-unknown}) vs main ($MAIN_HEAD_SHA) after $MAX_REBASE_ATTEMPTS attempts, and no live author session exists (ga-it1of liveness check: ${REBASE_LIVENESS_TRACE:-none}). Marker permanently parked at gate-status:error. Source bead $BEAD_ID: $(gate_needs_human_clause "$_NH_STATUS")" 2>/dev/null || true
          if [ -n "$BEAD_ID" ]; then
            bd -C "$BEAD_CITY" label remove "$BEAD_ID" "story:in-flight"            -q 2>/dev/null || true
            bd -C "$BEAD_CITY" label remove "$BEAD_ID" "pilot:dispatched"           -q 2>/dev/null || true
          ...
          # instead of escalating. ga-a6etc2: "stay in the retry queue" is now
          # BOUNDED — at GATE_CLEAN_RETRY_HARD_CAP total failed attempts control
          # takes the else below and parks the marker at needs-rebase; between
          ...
          # ga-a6etc2, (b) a proven-clean marker that used up
          # GATE_CLEAN_RETRY_HARD_CAP. Attempt counts below use $NEXT_ATTEMPT (the
          # real number of failures), not $MAX_REBASE_ATTEMPTS: with (b) they differ.
          _CAP_NOTE=""
...
These are almost certainly ghost duplicates of a correctly-placed file one level up (e.g. .gascity-gastown-hq/.gascity-gastown-hq/foo instead of .gascity-gastown-hq/foo) and are never read by any deploy/launchd job. Verify 'git rev-parse --show-toplevel' resolves to the true repo root and target paths relative to THAT before building; remove the ghost path(s); re-run /gate-done." 2>/dev/null || true
  ...
  _NH_STATUS="failed"
  [ -n "$BEAD_ID" ] && _NH_STATUS=$(gate_apply_needs_human "$BEAD_CITY" "$BEAD_ID" "gate:needs-human:technical")
  [ "$_NH_STATUS" != "armed" ] && warn "gate:needs-human FAILED TO APPLY on $BEAD_ID after retry (ga-ir033/ga-36ta4) — circuit-breaker NOT armed."
  if [ -n "$BEAD_ID" ]; then
    bd -C "$BEAD_CITY" label remove "$BEAD_ID" "story:in-flight"            -q 2>/dev/null || true
    bd -C "$BEAD_CITY" label remove "$BEAD_ID" "gate:reviewing"             -q 2>/dev/null || true
    bd -C "$BEAD_CITY" label remove "$BEAD_ID" "pilot:dispatched"           -q 2>/dev/null || true
...
$DOUBLE_PREFIXED_FILES
$(gate_needs_human_clause "$_NH_STATUS") story:in-flight + gate:reviewing + pilot:dispatched stripped (Pilot lane slot freed). Fix the build topology, remove the ghost path(s), resubmit." 2>/dev/null || true
  fi
  gc --city "$GC_CITY" mail send mayor \
...
$DOUBLE_PREFIXED_FILES
$(gate_needs_human_clause "$_NH_STATUS") This class of bug lets a ghost file pass review while the real target file stays untouched (false-done)." 2>/dev/null \
...
$DOUBLE_PREFIXED_FILES
This is the signature of a mis-rooted nested-worktree build (git-worktree-nested-city-path-prefix-gotcha) — the real target file was probably never touched. $(gate_needs_human_clause "$_NH_STATUS") Verify 'git rev-parse --show-toplevel', fix the ghost path(s), and resubmit." \
