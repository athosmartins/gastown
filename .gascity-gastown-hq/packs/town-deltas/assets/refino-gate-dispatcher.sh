...
# `set -euo pipefail` (L56) — a POSIX special builtin dies immediately on a
...
_REFINO_GATE_QHC_SIB="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/quiet-hours-check.sh"
if [ -r "$_REFINO_GATE_QHC_SIB" ]; then
  source "$_REFINO_GATE_QHC_SIB"
fi
unset _REFINO_GATE_QHC_SIB
...
if [ "$(_quiet_hours_blocks)" = "1" ]; then
  ...
  log "Quiet-hours signal UNREADABLE (stale/corrupt ${QUIET_HOURS_LEVEL_FILE:-unset}) — fail-open, refino-gate proceeding normally this sweep (ga-dxyvxr)."
fi
