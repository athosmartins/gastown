...
#                     removed, story:done NOT set, author + mayor nudged, exit != 0.
...
[ "$RUN_RC" -ne 0 ] && ok "S2 STALE → block exits non-zero (halts)" || nok "S2 exit" "rc=$RUN_RC"
...
[ "$RUN_RC" -ne 0 ] && ok "S3 UNVERIFIABLE → block exits non-zero (fail-closed)" || nok "S3 exit" "rc=$RUN_RC"
